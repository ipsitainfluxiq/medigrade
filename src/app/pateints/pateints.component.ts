...
export class PateintsComponent implements OnInit {
  ...
  check_uncheck_all1(){
    console.log('this.check_uncheck  '+this.check_uncheck);
    if(this.check_uncheck == null || this.check_uncheck == false || this.check_uncheck == 0){
      this.check_uncheck = true;
    }else{
      this.check_uncheck = false;
    }
    setTimeout(() => {
    this.check_uncheck_all();
    }, 50);
  }
  check_uncheck_all() {
    setTimeout(() => {
      ...
      .subscribe(res => {
        ...
        if (result.status == 'success' && typeof(result.id) != 'undefined') {
         ...
         // this.framesrc='https://medigradehealth.com/assets/recorder/uploads/'+this.patientrecorddetails.patientid+'.wav';
          this.framesrc='https://'+this.serverhost+'/assets/recorder/uploads/'+this.patientrecorddetails.patientid+'.wav';
